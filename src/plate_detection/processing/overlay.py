...
# デフォルトバナーパス
DEFAULT_BANNER_PATH = (
    Path(__file__).parent.parent.parent.parent / "assets" / "banner_sample.png"
)
...
def overlay_with_alpha(
    base: np.ndarray, overlay: np.ndarray, x: int, y: int, opacity: float = 1.0
) -> np.ndarray:
    """アルファブレンドでオーバーレイ"""
    result = base.copy()
    ...
    if overlay_crop.shape[2] == 4:
        ...
        blended = (overlay_crop[:, :, :3] * alpha + base_crop * (1 - alpha)).astype(
            np.uint8
        )
...
def overlay_banner(
    ...
    if mode == "overlay":
        ...
        resized_img = cv2.resize(
            image, (new_img_w, new_img_h), interpolation=cv2.INTER_AREA
        )
        ...
        if position == "bottom":
            ...
            result[y_offset : y_offset + new_img_h, x_offset : x_offset + new_img_w] = (
                resized_img
            )
