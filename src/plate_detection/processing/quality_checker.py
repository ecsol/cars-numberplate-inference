...
def check_mask_completeness(
    ...
    if info["edge_density_before"] > 0:
        info["reduction_ratio"] = 1 - (info["edge_density_after"] / info["edge_density_before"])
    else:
...
def verify_no_plate_leak(
    ...
    for i, region in enumerate(detection_regions):
        ...
        if edge_density > edge_threshold:
            return False, f"領域{i+1}: エッジ密度が高すぎます ({edge_density:.3f} > {edge_threshold})"
        
        if texture_variance > variance_threshold:
            return False, f"領域{i+1}: テクスチャ分散が高すぎます ({texture_variance:.1f} > {variance_threshold})"
