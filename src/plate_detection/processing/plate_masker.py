...
# デフォルトマスク画像パス
DEFAULT_MASK_PATH = (
    Path(__file__).parent.parent.parent.parent / "assets" / "plate_mask.png"
)
...
def create_default_mask(
    width: int = 200, height: int = 100, color: Tuple[int, int, int] = (255, 255, 255)
) -> np.ndarray:
    """
    デフォルトのマスク画像を生成
...
def calculate_iou(box1: np.ndarray, box2: np.ndarray) -> float:
    ...
    Args:
        box1: (x, y, w, h) 形式のボックス
        box2: (x, y, w, h) 形式のボックス
...
def filter_overlapping_detections(
    ...
    """
    重複する検出を除去（NMS - Non-Maximum Suppression）

    同じプレートが2つ以上検出された場合、confidenceが高い方を残す。

    Args:
        detections: 検出結果のリスト [{'mask': np.array, 'confidence': float, ...}, ...]
        iou_threshold: IoUがこの値以上の場合、重複とみなす
