...
class DetectionItem(BaseModel):
    ...
    ocr_score: Optional[float] = Field(
        default=None, example=0.85, description="OCR信頼度（ocr_check=true時のみ）"
    )
...
class DetectItem(BaseModel):
    ...
    confidence: float = Field(example=0.95, description="検出信頼度（0.0〜1.0）")
    mask_points: List[List[float]] = Field(
        description="セグメンテーションポリゴンの座標リスト"
    )
...
class DetectResponse(BaseModel):
    ...
    count: int = Field(example=1, description="検出数")
    detections: List[DetectItem] = Field(
        description="検出されたナンバープレートのリスト"
    )
...
async def predict(
    ...
    mask_mode: str = Form(
        "fill", description="マスクモード: fill（白塗り）/ image（画像）"
    ),
    low_light_fix: bool = Form(False, description="低照度画像の自動補正"),
    quality_check: bool = Form(
        False, description="マスク品質チェック（未マスク流出防止）"
    ),
):
    """
    ナンバープレートを検出してマスク処理
    ...
    if auto_rotate:
        # 回転検出を試みる
        valid_detections, best_img, rotation_used = try_detect_with_rotations(
            detector, img
        )
    else:
        # 通常検出のみ
        ...
        valid_detections = [
            d for d in detections_raw if validate_aspect_ratio(d["mask"])
        ]
        best_img = img
        rotation_used = "original"
    ...
    for det in valid_detections:
        ...
        if mask_mode == "image":
            # 射影変換でマスク画像を合成
            result_img = mask_plate_with_image(
                result_img, polygon, MASK_IMAGE, opacity=1.0
            )
        else:
            # 白塗り（デフォルト）
        ...
        detection_info = {"confidence": det["confidence"], "polygon": quad_points}
