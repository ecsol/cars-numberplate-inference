...
from src.plate_detection.processing.plate_masker import (
    mask_plate_with_image,
    load_mask_image,
    filter_overlapping_detections,
)
from src.plate_detection.processing.plate_verifier import verify_plate_with_ocr
...
def process_image(
    ...
    Args:
        ...
        confidence: 検出信頼度閾値
        verify_ocr: OCRで検証（数字がある場合のみマスク）
    ...
    # OCR検証（数字が含まれている場合のみマスク）
    if verify_ocr and valid_detections:
        ocr_verified = []
        for det in valid_detections:
            polygon = det["mask"]
            is_valid, ocr_text, ocr_conf = verify_plate_with_ocr(image, polygon)
            if is_valid:
                det["ocr_text"] = ocr_text
                det["ocr_confidence"] = ocr_conf
                ocr_verified.append(det)
            else:
                print(f"  [OCR Filter] Rejected: '{ocr_text}' (conf: {ocr_conf:.2f})")
        valid_detections = ocr_verified

    # 重複検出を除去（同じプレートが2回検出されるのを防ぐ）
    # iou_threshold=0.3 で厳しくフィルタリング
    valid_detections = filter_overlapping_detections(
        valid_detections, iou_threshold=0.3
    )
...
使用例:
  ...
  python scripts/process_image.py --input=folder --is-masking=false
    ...
    parser.add_argument(
        "--output",
        help="出力先（省略時: フォルダ→input/output, ファイル→input_masked.jpg）",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="false",
        help="OCR検証（数字がある場合のみマスク）(true/false)",
    )
    ...
    verify_ocr = args.verify_ocr.lower() in ("true", "1", "yes")
    ...
    if input_path.is_dir():
        ...
        try:
            ...
            print(
                f"  サイズ: {result['original_size'][0]}x{result['original_size'][1]}"
            )
